...
int _write(int fd, char* buf, int size)
{
	...
	uart_dma_write_bytes(uart_dma_1, (uint8_t*)buf, (uint16_t)size);
#elif(DEBUG == DEBUG_UART_DMA_2)
	uart_dma_write_bytes(uart_dma_2, (uint8_t*)buf, (uint16_t)size);
#elif(DEBUG == DEBUG_UART_DMA_3)
	uart_dma_write_bytes(uart_dma_3, (uint8_t*)buf, (uint16_t)size);
#elif(DEBUG == DEBUG_UART_DMA_4)
	uart_dma_write_bytes(uart_dma_4, (uint8_t*)buf, (uint16_t)size);
