...
// 0x7FF0:  RTMI Period (in microseconds)
...
void spi_write_reg(uint8_t addr, uint16_t data)
{
...
uint16_t spi_read_reg(uint8_t addr)
{
	uint16_t read_val;
...
void on_rtmi_int(void)
{
	...
	{
		read_val = spi_read_reg(rtmi_channels[trigger_channel]);
		...
		switch(trigger_mode)
		{
			...
			case 0x02:	//signed greater than
				if((int32_t)read_val > (int16_t)rtmi_threshold)
					rtmi_control |= 0x2000;	//set Active bit
				break;
	...
	if(rtmi_control & 0x6000)	//triggered or in continuous sampling mode
	{
		...
		{
			read_val = spi_read_reg(rtmi_channels[channel_idx]);
...
int main(void)
{
	...
	gpio_set_mode(GPIOB, GPIO_DIR_SPD_IN | GPIO_MODE_PULL_IN, GPIO_PIN_4);	//SPI1_MISO, DIAG0, DIAG1
	gpio_set_pin(GPIOA, GPIO_PIN_15);	//set SPI1_NCS
	...
	spi_init(SPI1, SPI_8_BIT | SPI_CLK_DIV_8 | SPI_MODE_1);	//12 MHz
    ...
    while(1)
    {
    	if((cdc_bytes_available() >= 4) && (cdc_peek() & 0x80))	//Handle write datagram
		{
			...
			{
				spi_write_reg(datagram[1] | 0x80, datagram_val);
			}
		}
    	...
    	if((cdc_bytes_available() >= 2) && !(cdc_peek() & 0x80))	//handle read datagram
		{
			...
			if(datagram[0] & 0x7F)	//Handle access to FW register
			{
				switch(datagram[1])
				{
					...
					case 0xFD:	//unique_id_l
						...
						datagram_val = (uint16_t)(*(volatile uint32_t*)0x1FFFF7E8);	//These two do the same thing.. ok to read as 16-bit words
						break;
					case 0xFC:	//unique_id_h
						//datagram_val = *(volatile uint16_t*)0x1FFFF7EA;
						datagram_val = (uint16_t)((*(volatile uint32_t*)0x1FFFF7E8) >> 16);
						break;
					case 0xFB:	//RTMI Control
			...
			{
				datagram_val = spi_read_reg(datagram[1] & 0x7F);
				datagram[0] = 0x80;
				datagram[1] = ((uint8_t*)&datagram_val)[1];
				datagram[2] = ((uint8_t*)&datagram_val)[0];
				cdc_write_bytes(datagram, 3);
